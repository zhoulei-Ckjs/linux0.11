...
/**
 * @brief 获取访问权限。
 * @details "\004\002\006\377" 表示八进制 004 002 006 337。""括起来表示字符串，字符串里面是8进制字符，用二进制表示就是 0b00000100 0b00000010 0b00000110 0b11011111。
 * @return 004（MAY_READ）：如果 x 为只读，即 x == 0，那么 ACC_MODE(x) 则取 "\004\002\006\377"[0]，即取 004
 * @return 002（MAY_WRITE）：如果 x 为只写，即 x == 1，那么 ACC_MODE(x) 则取 "\004\002\006\377"[1]，即取 002
 * @return 006（MAY_READ | MAY_WRITE）：如果 x 为可读写，即 x == 2，那么 ACC_MODE(x) 则取 "\004\002\006\377"[2]，即取 006
 * @return 337（八进制转十六进制就是 0xFF，表示错误）：如果 x == 3，那么 ACC_MODE(x) 则取 "\004\002\006\377"[3]，即取 337
 * @note 这种写法叫查表法，正常写法是 if else 的分支预测，这样没有分支预测了，会更快。
 */
